// Package file contains Daily Coding Problem #17
//
...
// isFile return true if str contains atleast one '.'
func isFile(str string) bool {
	return len(strings.Split(str, ".")) > 1
}

// GetMaxPathLength returns the maximum path-length given a path string
func GetMaxPathLength(pathString string) int {
	// Assume that no more than 30 subsirs
	path := make([]string, 32)
	maxLength := 0
	for _, item := range strings.Split(pathString, "\n") {
		strippedItem := strings.Split(item, "\t")
		n := len(strippedItem) - 1
		path[n] = strippedItem[n]
		if isFile(strippedItem[n]) {
			file := strings.Join(path[:n+1], "/")
